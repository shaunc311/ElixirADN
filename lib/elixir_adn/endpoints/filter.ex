defmodule ElixirADN.Endpoints.Filter do
	...
	def get(user_token, filter_id) when is_binary(filter_id) do
		result = process_get_filter(user_token, filter_id)
		case result do
			{:error, _} -> result
			_ -> {:ok, parse_to_filters(result)}
		end
	...
	#make the get call with the auth token
	defp process_get_filter(user_token, filter_id) do
		Http.call({:get, "https://api.app.net/filters/#{filter_id}"}, [{"Authorization", "Bearer #{user_token}"}])
	end
